\begin{frame}{Oppgave 2 H2017}
    \begin{block}{(5\%) a) Forenkle uttrykket $\Theta(n)+O(n^2) + \Omega(n^3)$}
    \begin{itemize}
        \item Vi får $[n, n] + [1, n^2] + [n^3, \infty) = [n^3, \infty] = \Omega(n^3)$
    \end{itemize}
    \end{block}
    \begin{block}{(5\%) b) Forenkle uttrykket $\Theta(n+\sqrt{n})+O(n^2+n) + \Omega(n\cdot(n+\lg{n}))$}
    %Lenke til LF: \url{https://algdat.idi.ntnu.no/arkiv/2017.des.tdt4120.losn.no.pdf}
    %Løsning:
    \begin{itemize}
        \item<2-> Vi forenkler: $\Theta(n+\sqrt{n}) \equiv \Theta(n)$, $O(n^2+n) \equiv O(n^2)$, $\Omega(n\cdot (n+\lg{n})) \equiv \Omega(n^2)$\\
        \item<3-> Vi får $[n, n] + [1, n^2] + [n^2, \infty) = [n^2, \infty] = \Omega(n^2)$
    \end{itemize}
    \end{block}
\end{frame}